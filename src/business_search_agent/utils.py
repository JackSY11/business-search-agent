...
async def business_search_async(
    query: str, max_results: int = 20, config: Optional[BusinessConfig] = None
) -> Dict:
    """
...
def extract_high_quality_results(results: Dict, min_score: float = 70.0) -> List[Dict]:
    ...
    return [r for r in results["results"] if r.get("content_quality_score", 0) >= min_score]
