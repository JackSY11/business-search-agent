...
class BusinessConfig:
    ...
    def _get_performance_config(self) -> Dict:
        ...
        return {
            ...
            'parallel_enabled': os.getenv('PARALLEL_ENABLED', defaults['parallel_enabled']).lower() == 'true',
            'connection_pool_size': int(os.getenv('CONNECTION_POOL_SIZE', defaults.get('connection_pool_size', 20)))
        }
    ...
    def _get_business_config(self) -> Dict:
        ...
        return {
            ...
            'content_quality_threshold': float(os.getenv('QUALITY_THRESHOLD', defaults['quality_threshold'])),
            'chinese_content_threshold': float(os.getenv('CHINESE_THRESHOLD', defaults['chinese_threshold'])),
            'zhihu_boost_enabled': os.getenv('ZHIHU_BOOST', defaults['zhihu_boost']).lower() == 'true',
